...
use ray_tracer::{
    view_transform, BlendedPattern, Camera, Canvas, Light, PerturbedPattern, Plane,
    RingPattern, Shape, SolidPattern, Sphere, StripePattern, Transform, World,
};
...
impl PatternImage {
    pub fn new() -> PatternImage {
        ...
        *s2.transform_mut() = scale(&DMat4::identity(), &vec3(0.1, 0.1, 0.1)) * rotate_y(&DMat4::identity(), PI / 2.);
        ...
        *middle.material_mut().pattern.transform_mut() = scale(&DMat4::identity(), &vec3(0.2, 0.2, 0.2));
        middle.material_mut().diffuse = 0.7;
        middle.material_mut().specular = 0.3;

        let mut right = Sphere::new(world.counter.increment());
        *right.transform_mut() =
            translate(&DMat4::identity(), &vec3(1.5, 0.5, -0.5)) * scale(&DMat4::identity(), &vec3(0.5, 0.5, 0.5));
        ...
        *left.transform_mut() =
            translate(&DMat4::identity(), &vec3(-1.5, 0.33, -0.75)) * scale(&DMat4::identity(), &vec3(0.33, 0.33, 0.33));
